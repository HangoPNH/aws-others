##################################
### DIAC MSK Cluster CPU Usage ###
##################################

# cpu usage 3rd rule
resource "aws_cloudwatch_metric_alarm" "diac_msk_cluster_critical_cpu_utilization_alarm" {
  ...
  alarm_name          = "[${title(var.infra_env)}] [MSK] Broker ${each.value} - CPU Usage [CRITICAL]"
  alarm_description   = "[Critical] CPU Usage of Broker ${each.value} exceeds ${var.msk_cluster_cpu_threshold_3}%"
  ...
  threshold           = var.msk_cluster_cpu_threshold_3
  statistic           = "Average"
  ...
  tags = merge(local.common_tags,
    {
      Name = "${var.project_key_tag}-alarm-msk-cluster-critical-cpu-usage-${var.repo_name}-${var.infra_env}"
    }
  )
}
